...
Build a "Lab Drop" marketing one-off: a short (~50-60 s) promo video for a
...
1. **Brand sting (~0-8 s):** the ONLY place "ATC LAB DROP" appears, ever.
...
2. **whoami cold open:** the terminal types `$ whoami` (or the platform's
   equivalent, e.g. `PS> $env:USERNAME`, `Router# show users`) and answers with
...
4. **3-6 numbered slides** (`01 / 05` mono indicator + progress dots): one
   headline per slide summarizing a course or skill area, each with a small
   animated visual (terminal, stack diagram, chips + bars). Slide beats come
   from the narration transcript, same as path videos. Slide headlines and
   labels are on-screen text, so they follow `.claude/style-guide.md`: no em
   or en dashes, "and" or "plus" instead of "+" or "/" in words, a space
   before units ("90 s"), and no emojis.
5. **Outro:** bare `$` prompt → pixel-mosaic wipe → end card: learning-path
   wordmark + accent bar + the WWT mark (`public/labdrop/wwt-logo.png`)
   floating gently (±5 px, ~6 s period). No CTA unless asked.
...
- `particles-loop.mp4`: full-video backdrop (Envato dark line-particles,
  20 s, use `<Loop durationInFrames={595}>`).
- `glitch-intro.mp4`: behind the sting only, with a `rgba(5,8,15,0.55)`
  ...
  frame without it).
- `music.mp3`, `sfx/` (impact, click, typing, whoosh): the source library is
...
- **Music beats**: detect real onsets before animating the sting. Decode
  the track's first ~14 s to mono 8 kHz WAV (`npx remotion ffmpeg`, wav not
  raw pcm; the bundled ffmpeg has NO filters and no aevalsrc/anoisesrc),
  compute 20 ms RMS windows in Node, peak-pick positive RMS deltas. Slam one
...
- Fixed-duration composition (no `calculateMetadata`); duration =
  `TL.end * FPS`. Keep the ~2 s foreground-fade end buffer (backdrop holds).
...
1. Confirm the learning path name, slide content (course and skill areas), and
