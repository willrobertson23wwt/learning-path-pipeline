...
speaking time is `length` minus the 2 s end buffer, minus about 0.2 s per scene
change, minus about 1.5 s for each key animation the visual brief wants the
narration to stop for (the holds `/video` makes there). Use 140 words per minute for an
...
- Start a new paragraph at each scene change, and end a sentence right
  ...
  `/video` splits the narration at those sentence boundaries for holds (a
  short breath at a still scene change, the animation's run time plus about
  1 s around a key animation). Write
  sentences that make sense on either side of a gap; background music fills
  it in post. Mark each key-animation hold in the visual brief.
